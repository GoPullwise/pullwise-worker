...
def review_prompt(job: dict) -> str:
    ...
    return (
        ...
        "inside this checkout. Stage 1: enumerate as many plausible issue candidates as the repository context "
        "supports, including candidates that still need proof. Stage 2: reproduce or verify each candidate one by "
        "one with focused commands, static checks, or minimal tests inside the checkout. For a reportable issue, "
        "include runtime reproduction evidence in issue_cards.evidence: "
        "the exact command, exitCode, either substantive output or a redacted logPath, and a repository-relative "
        "file/startLine tied to the issue. The worker will filter out candidates that lack reproducible runtime evidence, "
        "so keep rejected or inconclusive candidates clearly marked with verification_results verdicts `rejected` or "
        "`inconclusive` instead of presenting them as confirmed. "
        ...
        "`audit_protocol`, `issue_cards`, and `verification_results`. Do not return top-level "
