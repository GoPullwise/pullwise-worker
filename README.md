...
When the server includes per-job `agentConfig`, the worker applies the job's Codex `reasoningEffort` and OpenCode `variant` for that review without changing the process-wide defaults.
