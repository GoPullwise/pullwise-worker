...
有 7 个 pending（其中 D2 inactive）、20 个 resolved，唯一 active decision 是 D21。
S2-S5 的 decision gate 已无 pending blocker；本文不把已决选项当成实现完成证据。
...
- decision register 为 `valid_pending`，7 个 pending（其中 D2 inactive）、20 个 resolved；
  active decision 为 D21，S2-S5 的 decision gate 已无 pending blocker。
