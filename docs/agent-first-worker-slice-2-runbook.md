...
的 Task/Attempt 分层语义，但在 S4 实现证据通过前不改写当前 shadow/runtime。当前 register
另以 custom `new_tasks_only` 解决 D24，resolution digest 为
...
digest 保持无环 DAG。D26 已选择 recommended `roadmap_separate_designs`，resolution
digest 为 `ce8a907836b3b8209f12f7c48f66878e9534d7cac667532c2899f3d74c86602f`；未闭合
远期版本只属 roadmap，每版开工前须另写完整 implementation design。当前 register 有
2 个 pending（其中 D2 inactive）、25 个 resolved，唯一 active decision 是 D22。
S2-S5 的 decision gate 已无 pending blocker，S6/S7/S8 均仅由 D22 阻断；本文不把
已决选项当成实现完成证据。
...
- 当前 pre-D8 shadow reducer 的 `outer_lease.fenced` 会把 Task 写成
  ...
  D10 已选择全局 safety-first 穷举矩阵；矩阵完整冻结前不得实现候选 precedence。
  缺失的 abandonment contract 仍遵守 Slice 1
  `SPEC_GAP-TRANSPORT-CONTRACTS`，没有猜造 wire bytes；S4 实现证据闭合前不得修改或提升
...
## Legacy 单槽接入与回滚（协调切换前的当前状态）

本节只记录现有 scaffold，不定义 D20 的生产目标。协调切换后新 Gate 立即成为唯一
生产权威；旧 QA 不作为 hard floor，也不保留 production shadow、fallback、
downgrade 或双轨共存。
...
- decision register 为 `valid_pending`，2 个 pending（其中 D2 inactive）、25 个 resolved；
  active decision 为 D22，S2-S5 无 pending blocker，S6/S7/S8 均仅由 D22 阻断。
...
| `tests/test_agent_first_decision_register.py` | 387 | D5-D8/D27 resolution/digest gate |
| `tests/test_agent_first_decision_register_gate.py` | 399 | slice blocker and normative reference gate |
| `contracts/agent-first/spec-decision-register.json` | 454 | machine decision source |
...
| `docs/agent-first-worker-mvp-implementation-design.md` | 1813 | D5-D8/D27 normative state semantics |
| `docs/agent-first-worker-spec-decision-register.md` | 581 | generated decision view |
| `docs/agent-first-worker-slice-1-runbook.md` | 226 | D1-D8/D27 gate 状态同步 |
