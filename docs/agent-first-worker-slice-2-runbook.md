...
有 6 个 pending（其中 D2 inactive）、21 个 resolved，唯一 active decision 是 D23。
S2-S5 的 decision gate 已无 pending blocker，S6 仅由 D22 阻断；本文不把已决选项当成实现完成证据。
...
- decision register 为 `valid_pending`，6 个 pending（其中 D2 inactive）、21 个 resolved；
  active decision 为 D23，S2-S5 无 pending blocker，S6 仅由 D22 阻断。
