...
有 8 个 pending（其中 D2 inactive）、19 个 resolved，唯一 active decision 是 D20。
S2-S4 的 decision gate 已无 pending blocker；本文不把已决选项当成实现完成证据。
...
- decision register 为 `valid_pending`，8 个 pending（其中 D2 inactive）、19 个 resolved；
  active decision 为 D20，S2-S4 的 decision gate 已无 pending blocker。
