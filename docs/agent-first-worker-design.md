...
片段中的 TaskResult 为排版省略了第 5.3 节字段；真正合并时必须放完整不可变 envelope，而不是只发摘要。两个 digest 位于 sibling `task_result_integrity`，不属于任何被哈希对象。为消除 terminal debug receipt 的哈希循环，协议同时定义 `TaskResultCore`：它是完整 TaskResult 删除 `diagnostics.worker_debug_fragment` 后的 JCS 对象；`task_result_core_digest = sha256(JCS(TaskResultCore))`，完整 `task_result_digest = sha256(JCS(TaskResult))`。前者在 terminal fragment 冻结前固定，后者在 receipt 写入后固定。Server 需要重新计算两者，校验 `task_id == scan_id`、run/job/transport attempt、lease、schema major、outcome 与现有 terminal status 的映射，再保存原始 envelope。Web 永远不直接读取该 extension。
...
- Task Request digest、Requirement Ledger digest、Completion Proposal、`TaskResultCore` digest/ref、ObservationManifest、Attestation 元数据与任务工程 artifact manifest；原始大对象只保留内容寻址 hash/ref。Worker fragment 绝不嵌入含 debug receipt 的最终 TaskResult 字节，也不把自身当作任务 artifact；已接受的完整 TaskResult digest/ref 由 ServerDebugSnapshot 收录。
...
- 已接受的最终 TaskResult digest/ref、对应 TaskResultCore digest、诊断 receipt/binding，以及 result、requirement counts、artifact 和 event 的通用 reconciliation。
...
2. Server 完成 scope/schema/source hash/redaction 校验并原子持久化后，返回绑定完整幂等键、`source_sha256 + task_result_core_digest` 的 `server_receipt_id`；相同绑定的上传重试必须返回同一 receipt ID。
3. 只有 `uploaded` 分支把 receipt 写入 `diagnostics.worker_debug_fragment`，再计算完整 TaskResult digest 并提交。Server 重新计算 core/full digest，并规定一个 receipt **只能绑定一个完整 TaskResult digest**；相同 result checksum/digest 的网络重试幂等成功，不同 digest 的复用冲突拒绝。
4. `local_only|unavailable` 分支不创建或绑定 receipt；Worker 在写入 state/reason、清空 `server_receipt_id` 与 Server fragment ref 后直接计算完整 TaskResult digest。Server 必须断言这些 Server 字段为空并跳过 receipt binding，不能因 Debug fail-open 拒绝诚实工程终态。
5. Composer 只把“被已接受 TaskResult 引用、且 core digest 与该 TaskResult 重算值一致的 sealed receipt”作为 complete terminal Worker contributor。完整 TaskResult 由 Server snapshot/Assembly 收录；后续 cleanup 日志只能进入独立 postlude fragment 或 Server event，不能改写 terminal fragment、TaskResult 或既有 Assembly。

仅对携带有效 `agent_task_protocol` extension 的新结果，Terminal result ingest 的唯一线性化点是一个数据库事务/CAS：先只读校验完整 v1 envelope、result checksum、transport/native fence、TaskResult core/full digest、EvidenceClosureManifest、artifact manifest 和 outcome 映射。若 diagnostics state 为 `uploaded`，还必须校验唯一 receipt scope，并在同一提交中执行 `receipt: unbound -> task_result_digest`；若为 `local_only|unavailable`，则必须断言 `server_receipt_id` 与 Server fragment ref 为空并完全跳过 binding。随后同一提交插入唯一 terminal result/raw envelope/checksum，并推进 job/reviewRun/scan 终态与公开 projection。任一校验或写入失败全部回滚，不能先绑定 receipt 造成 poison。若唯一结果已是相同 `result_checksum + task_result_digest + (receipt_id|null)`，重试返回原成功响应；任一分量不同均为冲突。
