...
observations 表，也没有 production composition-root 接线。D28-D30 均 resolved，S3
decision gate 已 ready；本文中的 injected interfaces 和内部 tracer 不实现或替代这些决议，
也不把 ready 状态当作 package/runtime 实现证据。
...
不会直接暴露，而是进入 injected unavailable/violation committer。D30 已冻结生产
grant/intent/receipt/budget 的唯一 durable linearization；该 seam 并未实现 D30 contract。
...
- D28 已选择 `logical_bundle_generated_wrappers`（resolution digest
...
- D30 已选择 `worker_journal_server_authority`（resolution digest
  ...
  与 budget consumption。必须冻结 begin/settlement/abandon/replay/crash fixtures，并把 local
  tool receipt 与 Server transport receipt 分型隔离。该决议不表示 journal/runtime 已实现。
...
当前预期结果为 `ready`：register 本身 valid，29 resolved、0 applicable pending、D2
inactive 且无 active question；S3 没有 pending-decision blocker。该结果不表示 Server-owned
package、Worker current-only journal、production composition 或 S3 runtime 已实现。

D27 ratchet 可单独复核：

    python scripts/verify_agent_first_legacy_absence.py --workspace-root ..
    python -m unittest \
      tests.test_agent_first_legacy_absence \
      tests.test_agent_first_legacy_absence_hardening
...
当前本地 D27 测试证据为 Ran 35 tests，OK；production strict 观测保留 109 个
present surfaces，自引用项不生成确定性 failure，其余 108 项仍生成
`legacy_surface_present`，并产生 1 个上述 indeterminate reason。组合回归同时锁定
indeterminate verdict 不得吞掉独立 registered 或 unexpected legacy failures。
...
- scripts/verify_agent_first_legacy_absence.py：267 行，D27 default/strict verdict。
- test_agent_first_legacy_absence.py：296 行，一般 ratchet/strict 语义。
- test_agent_first_legacy_absence_hardening.py：398 行，production catalog fail-closed。
...
唯一的生成包装器例外是 `pullwise_worker/_generated_agent_task_contract.py`：来源固定为 `pullwise-server@a2538e7a5fa1cba2a35a9ea9e076501ac719570f:pullwise_server/agent_first_contract_bundle_python.py`，固定为 6970 行及 SHA-256 `a8a4bb9228be3c0179ee60958c9f08e2254e88377f149434753913b643243009`。它不是通配符或仅凭 marker 的豁免；文件缺失或降至 <=400 行时必须移除。
