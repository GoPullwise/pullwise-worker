...
- raw gitlink mapping 会被拒绝。可信 catalog 只能由固定 Git executable 对精确
  40-hex revision 生成；Git replace refs 与 lazy fetch 同时由命令行和环境关闭，
  catalog 绑定 checkout device/inode，scanner 打开的 root 必须匹配，gitlink
  目录不会被递归扫描。Git 低于 2.45 时命令 fail closed。
- catalog 与 scan 之间仍要求 materializer 保证 checkout 单写；Windows fallback
  仅是开发路径。生产 composition 未提供该保证前，不得启用本 tracer。
...
- excluded control path 在 snapshot/leaf open 前拒绝。dispatcher 只能消费 authority
  绑定的 capability 与 one-shot descriptor；读取上限为 expected size 加一，并再次
  验证 identity、size 和 hash。成功或失败都会关闭 descriptor；路径在 prepare 后
  被替换也不能把读取重定向到新目标。
...
- production materializer 对 Git >=2.45、exact-revision catalog 生成和 catalog/scan
  单写 checkout window 的接线与强制保证。
...
本地 Windows 证据：53 tests passed，4 个 POSIX/host-capability 用例按平台跳过。
...
- agent_kernel_gitlinks.py：202 行，exact-revision verified catalog。
- agent_kernel_gateway.py：400 行，固定顺序 orchestration。
- agent_kernel_r0_read.py：388 行，held-descriptor R0 read。
- test_agent_kernel_source_state.py：381 行。
- test_agent_kernel_gitlinks.py：212 行。
- test_agent_kernel_gateway.py：400 行。
- test_agent_kernel_r0_read.py：395 行。
- test_agent_kernel_r0_gateway.py：214 行。

AGENTS.md 与 MVP implementation design 都是已有 oversized 文档；本次只修订 durable
boundary/status evidence，没有向其中加入实现职责。
