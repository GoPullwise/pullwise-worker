...
| S3-S8 | 未开始 | 机器 decision register 当前为 `valid_pending`；只能询问 `active_decision_id=D8`，S3 gate 仍由 D11/D15/D16/D17 阻断 |
...
恢复结果必须满足：无双writer、无重复tool dispatch、预算与 deadline 均不回滚/延长、无跨进程裸 monotonic 比较、SourceState/Ledger一致、旧epoch fenced、terminal bytes不变。
