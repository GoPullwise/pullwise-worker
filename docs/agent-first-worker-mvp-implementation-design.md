...
| S3-S4 | S3a 内部只读 tracer primitives 已实现并完成本地安全终审；生产 S3/S4 未开始 | [S3a runbook](agent-first-worker-slice-3a-runbook.md)：SourceState/SourceDiff、exact-revision verified gitlink catalog、descriptor-rooted R0 read 与固定顺序 Gateway；它们不生成 versioned contract、Observation 或 durable journal。Git >=2.45 与 materializer 单写 checkout window 尚未接线；D23 Server package、current-only authority/budget/journal/CAS transaction 仍阻断生产接线 |
