...
| S3-S4 | S3a 内部只读 tracer primitives 已实现；生产 S3/S4 未开始 | [S3a runbook](agent-first-worker-slice-3a-runbook.md)：SourceState/SourceDiff、descriptor-rooted R0 read 与固定顺序 Gateway；它们不生成 versioned contract、Observation 或 durable journal。可信 exact-revision gitlink catalog、D23 Server package、current-only authority/budget/journal/CAS transaction 仍阻断生产接线 |
