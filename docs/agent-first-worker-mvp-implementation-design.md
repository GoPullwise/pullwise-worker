...
协调切换时的删除清单，不构成新协议验收证据。S3-S8 的 pending decision gates 仍须
逐项闭合；门禁允许生产切换前，不得提前改 runtime/schema/migration。
...
| S3-S8 | 未开始 | 机器 decision register 当前为 `valid_pending`；只能询问 `active_decision_id=D9`，S3 gate 仍由 D11/D15/D16/D17 阻断 |
...
| M-22 | D8 冻结 lease-loss 边界：外层 lease 丢失时 Task 保持 `ACTIVE` 或 `FINALIZING`，其 terminal/result 字段逐字节不变；精确的旧 transport/native Attempt 与 owner incarnation/session 被永久 fenced，并记录独立 transport abandonment 事实。只有满足显式恢复资格谓词的 successor 才可接管；MVP 不具备跨 lease successor 能力，D9/D10 仍分别决定最终终态 authority/linearization 与 precedence。 |
...
`outer_lease.fenced` 是 Attempt/transport-scope 的所有权丢失事实，不是 Task terminalization 事实。它必须在一个 Task 控制事件事务中阻止旧 transport、native Attempt、owner/session 的全部 event、artifact、tool 与 result 写，按 D5 令 `task_version` 恰好 `+1`；精确幂等重试返回原版本且不再次增版，任何 fence/abandonment/version 单边写都必须回滚。Task 只能保持当时的 `ACTIVE` 或 `FINALIZING`，既有 terminal/result bytes 不变。只有显式恢复资格谓词全部成立的 successor 才能取得新 ownership；MVP 未实现该跨 lease 能力，因此只能保持 fenced、停止写并等待兼容 transport 收敛，不能借此创建新 owner 或把 Task 改成 `TERMINAL`。QUEUED/WAITING 等无 active ownership 的 lease 事件以及最终无 successor 的终态 authority/linearization/precedence，不由 D8 推断，分别由后续状态契约与 D9/D10 闭合。
