...
协调切换时的删除清单，不构成新协议验收证据。D20 及其后续活动决策仍须逐项闭合；
门禁允许生产切换前，不得提前改 runtime/schema/migration。

## D9-D19 resolution overlay（Normative）

机器注册表已按用户授权依次解决 D9-D19，当前唯一活动问题是 D20。以下决议优先于
...
| S5-S8 | 未开始 | 机器 decision register 为 `valid_pending`，含 19 个 resolved、7 个 applicable pending 与 inactive D2；唯一活动问题为 `active_decision_id=D20` |
