...
<!-- BEGIN AGENT-FIRST DECISION REFS: MVP_CONTRACT_PACK -->
...
<!-- D11@sha256:dc65778d9f60563e39a9c3262200f8e26efd8c48c29aa0141087793186032a7e -->
<!-- D15@sha256:47cf85a523a63a4c26775fe6929bdd132fb37ac82f5cdda41128ee248827cb1b -->
<!-- D16@sha256:0acca8727c0044d5bc7ef7542e2bc51384c1ca56865889cec8e389b559403130 -->
<!-- D17@sha256:8f125e98166a1fa6edacc6ef2e29a1749eb13d5ab5d187d1aab63c38d5cac3a8 -->
<!-- D27@sha256:f3ef27ad6318d4da20d4750cdde9387b66045f1708a909b57aba1c6e48ec2b0e -->
...
<!-- D31@sha256:d6fe7e5184e410aa6d034be1b593c8bf83126d5af300ea489a3d077642b42254 -->
<!-- D32@sha256:11794116e7db5fdb330e001fa1ab7b7039ff1f1f04bc3283b9cddbc30bf3995e -->
<!-- D33@sha256:8bf9ed4ac35fdd2f0bfd790c1a8f8879776a44711683152921c9ae330e105fb4 -->
...
<!-- D39@sha256:85365d344a6bc0d36d5d11dbc088278722083bf51e98c9cd518dd3d57ac90f9c -->
<!-- D40@sha256:ce023be3c467370077f967bb7e17e9dee2064ea1fbe213a7a97b6815aec6cdc9 -->
<!-- D41@sha256:ccfa987beb48b1158a0122b13ed6bab40bd955e5142fbd1e0fc56f7151b1cca5 -->
<!-- END AGENT-FIRST DECISION REFS: MVP_CONTRACT_PACK -->
...
<!-- D6@sha256:e1ad16c135ae5f0880123becdd640bf685c0f201b44dd941830590b0b39174d8 -->
<!-- D7@sha256:5d7916e9389c0203185fb7e2e64be49df0ea52557d875f661f5d0180e093f5ea -->
...
<!-- D10@sha256:1daae4c66d41bd95a3eef8e24756590c8e6f75a05899548dc3126bfd39172e31 -->
<!-- D11@sha256:dc65778d9f60563e39a9c3262200f8e26efd8c48c29aa0141087793186032a7e -->
<!-- D12@sha256:b459cd0e371c34702e654761aa89caa21238f5c5314020e9d9c7484d60902764 -->
<!-- D13@sha256:4a90df4dce3840e2f726d952fa0b49ef9294e73e851208f968df30642720e5a7 -->
<!-- D14@sha256:1798cd24165aa5be17f5e5b256e3ecfd61a2f02e63d064e9f5da60edcf30a889 -->
<!-- D15@sha256:47cf85a523a63a4c26775fe6929bdd132fb37ac82f5cdda41128ee248827cb1b -->
<!-- D16@sha256:0acca8727c0044d5bc7ef7542e2bc51384c1ca56865889cec8e389b559403130 -->
<!-- D17@sha256:8f125e98166a1fa6edacc6ef2e29a1749eb13d5ab5d187d1aab63c38d5cac3a8 -->
<!-- D27@sha256:f3ef27ad6318d4da20d4750cdde9387b66045f1708a909b57aba1c6e48ec2b0e -->
...
<!-- D12@sha256:b459cd0e371c34702e654761aa89caa21238f5c5314020e9d9c7484d60902764 -->
<!-- D13@sha256:4a90df4dce3840e2f726d952fa0b49ef9294e73e851208f968df30642720e5a7 -->
<!-- D18@sha256:16fb38386dfedc25cbd4f7d3cc25aeeeb9512b3d0e3733fdb8591441eca3c8de -->
<!-- D19@sha256:0fb4d7e749fb873ccb7691ff2a87c30f2792969534311903ce439a5ac86c2796 -->
...
<!-- D31@sha256:d6fe7e5184e410aa6d034be1b593c8bf83126d5af300ea489a3d077642b42254 -->
<!-- D33@sha256:8bf9ed4ac35fdd2f0bfd790c1a8f8879776a44711683152921c9ae330e105fb4 -->
<!-- D34@sha256:2be5b5752b65714204fa6f41a0a126eb30e82bafcdeb38b5ece426938561158c -->
...
D27 显式 supersede D4 的 `legacy_v1` mapping 结论，并优先于本文后续所有
legacy/compatibility 文字。MVP 只实现一套 current Agent-First contract；不得实现
或保留 strict legacy Adapter、旧 result endpoint/outbox/DTO、双读写、shadow、
fallback、protocol downgrade、compatibility mode，或仅为旧数据存在的
...
mode/protocol，Worker 仅验证并执行，缺失、未知或不匹配时 fail closed。D24 的 cutover
决议把 Server 受审计的 Task acceptance/TaskRecord creation 屏障定为线性化
边界：屏障前暂停 intake，并将全部 pre-cutover Task 处置为权威终态、tombstone/delete
...
## D9-D38 resolution overlay（Normative）

机器注册表的冻结前缀已按用户授权解决 D9-D26；在该前缀原始闭合点上，D2 是唯一
inactive 决策且没有 applicable pending decision。当前 append-only register 中 D28-D38
均已解决，register 为 `ready`，共有 37 个 resolved、0 个 applicable pending，D2
inactive。D36 选择 `mvp_s3_s7_implementation_only_no_external_activation`；D37 已闭合
S4 accept/bootstrap 与双层 checkpoint/committed-manifest contract，消费恰好一次 Generate，
随后 S4 Task/Attempt/Owner、checkpoint、recovery 与 ACK tracer 已按 TDD 实现。S5 成功路径
测试又暴露 passed Success Gate、机械 selector、immutable Server authority/local Task version
chain 和真实 `FINALIZING -> TERMINAL` TaskResult CAS 的契约缺口。D38 已选择
`bounded_s5_terminal_control_and_selector_closure_one_generate_no_activation`，仅授权该 bounded
S5 closure，并仅在所有预生成门绿色后授权恰好一次 Generate；当前 D38 Generate count 为
`0`。不得伪造 success terminalization fact、让 caller 选择 outcome/reason、引入第二
authority/store/runner、legacy 补值、fallback、dual path 或 compatibility/downgrade shim。
以下已决结论仍优先于后文以“候选”或“待决”表述的旧段落，但不代表对应 runtime 已经实现：

- D9 以内部 TaskResult CAS 作为唯一语义终态线性化点；Server ACK 只确认可恢复
  transport projection，不能创建、替换或改写已提交 outcome。D10 要求以一张全局
  safety-first 穷举矩阵从权威事实复算唯一 outcome；完整矩阵冻结前，后文候选顺序
  仍不得直接实现。
- D11 使用 Worker-owned partial-delivery-manifest；D12 的修复必须创建新 generation
  和不可变 supersession journal，旧 generation 永不覆盖。在 D9 约束下，修复只能
  改变投影/envelope generation，不能改写已 CAS 的 TaskResultCore 或 outcome。
- D13 在内部发布前选择 CANCELLED；发布后只允许显式 reconciled variant/addendum，
  不改原 TaskResult。D14 用独立 BundleIntegrityManifest 绑定派生 bundle。
- D15 分离 Gate predicate 与 stable error registry；D16 删除 Q0 成功路径，full-scan
  至少 Q1；D17 用固定版本化 concern/coverage 表生成 slot ID。
- D18 在协调切换时使现有 root coordinator 原地成为唯一 current Task Owner，并将
  coordinator identity 与 owner incarnation 分层；不得形成并行第二控制器。D19 要求
  Owner incarnation 在 fanout 全程保持 live，并占用一个固定 agent/session slot。
...
- D24 选择 `new_tasks_only` 的 option-anchored custom 边界：以 Server 受审计的
  ...
  stop/fence/reject 不得冒充 Task terminalization 或 TaskResult。屏障后任何旧任务、
...
- D28 选择 `logical_bundle_generated_wrappers`（resolution digest
  `0a9c7e47ab03c92e5d48003ee3d7dc1b5df1cd68031fdd97dda7f85520297204`）：Server
  维护一份 canonical content bundle/root manifest，并从相同 canonical bytes 生成 Python
  与 npm 薄包装；两个 wrapper 共享同一逻辑 package identity/version/content digest。
  Worker/Web 分别 exact-pin wrapper version 与逻辑 digest，不得复制或重定义 schema；
  conformance 必须证明 wrapper 内容/digest 一致以及两个 consumer 的 exact lock。
- D29 选择 `layered_atomic_root`（resolution digest
  `dfe6c2e4b62226d5e7b155e2b7a51d04c94fd13905834b908e5d1b24f30eb5da`）：按
  authority/control、tool/evidence、budget、receipt/error 等内聚 family 分层维护 schema、
...
| S4 | 本地 current Task/Attempt/Owner bootstrap、双层 checkpoint、committed manifest、同 lease recovery 与 ACK tracer 已按 TDD 实现并定向验证 | D37 source closure 与恰好一次 Generate 已完成；Server accept/bootstrap/watermark 和 Worker 单一 current journal/control path 提供证据。该证据不等于 main-loop 切换或外部激活 |
| S5-S7 | D38 bounded S5 contract closure 正在进行；既有 non-success candidate 只证明 fail-closed 路径，S6/S7 仍依赖真实 S5 终态事务 | register 现有 37 个 resolved、0 个 applicable pending，D2 inactive；D38 Generate count 为 `0`。必须先以 TDD 闭合 passed Success Gate bridge、六轴 selector、单一 version authority 与真实 `FINALIZING -> TERMINAL` CAS，再在全部预生成门绿色后恰好一次 Generate；不得从 candidate tests 推断已实现、已激活或可部署 |
...
1. machine decision register 的已解决记录，尤其 D27 clean break；pending 决策仍保持阻断。
2. 本文对 MVP 的明确约束与状态/数据契约，但 D27 已标为删除清单的 legacy 文字除外。
3. 当前 Pullwise Worker 的 [项目规则](../AGENTS.md)，尤其单槽、无本地队列、Ubuntu 22.04、实例隔离、Server-owned policy 与安全不变量。
...
附录 A 的 legacy validator/fixtures 只用于识别删除范围，不再是实现权威或
兼容性发布门。
...
supersede 保持不变的旧记录。supersession 不重新激活旧问题 ID；
任何新适用的后续问题必须新增有序决策记录。受影响的规范单元必须引用当前
...
- R0/R1 通用内核能力；R2/R3/R4 在所有 profile 一律于 dispatch 前无外部副作用拒绝，并冻结空 Effect Ledger。
...
- R2/R3/R4 正向执行、非空 Effect Ledger、provider reconciliation。
...
| M-06 | MVP 只允许在同一外层 lease/grace 仍有效时恢复。外层 lease 丢失不得自称恢复；当前 Pullwise run-once Server 可把外层 run 收敛为 transport failure，但该兼容投影不是语义 Task 终态。 |
...
| M-21 | 恢复所需的时间状态只持久化整数 elapsed consumption。进程或 boot 局部的裸 monotonic origin、读数和 deadline 不持久化、不跨进程比较；每次启动从 immutable `absolute_deadline_at` 与 durable budget head 重建新的进程内 monotonic origin，任何墙钟异常或重复重启都不得扩大 deadline 或剩余预算。 |
| M-22 | D8 冻结 lease-loss 边界：外层 lease 丢失时 Task 保持 `ACTIVE` 或 `FINALIZING`，其 terminal/result 字段逐字节不变；精确的旧 transport/native Attempt 与 owner incarnation/session 被永久 fenced，并记录独立 transport abandonment 事实。只有满足显式恢复资格谓词的 successor 才可接管；MVP 不具备跨 lease successor 能力。D9 规定只有内部 TaskResult CAS 能形成语义终态，D10 规定由完整的全局 safety-first 矩阵选择 outcome。 |
...
- 所有持久化时刻为带 `Z` 的 UTC RFC3339，写入时精确到毫秒。墙钟用于审计，并在 Task 创建与恢复边界机械评估 immutable `absolute_deadline_at`；控制面不能用新的墙钟样本扩大既有 deadline。
- 进程内 elapsed time、timeout 与执行控制使用本进程的 monotonic clock；持久化层只保存整数 elapsed consumption。裸 monotonic origin、读数或 deadline 不得序列化，也不得在不同进程或 boot 间比较。
...
| `budget_entries` | PK `(task_id,budget_seq)` | reservation/consume/release 与 durable elapsed consumption；不保存可恢复的裸 monotonic 值 |
...
| `absolute_deadline_at` | immutable authoritative wall deadline；每次启动重建本地 monotonic origin 时必须绑定此值 |
| `terminalization_reserve_ms` | policy 值 |
| `completion_proposal_ref` | null |
| `final_observation_manifest_ref` | null |
| `terminal_kind` | null；非终态必须保持 null，已授权的 TaskResult 终态使用 `task_result`。D8 禁止把 transport abandonment 写入该字段；D9 禁止 Server ACK 或 transport loss 直接形成语义终态，D10 要求完整全局矩阵决定 outcome |
...
D6 冻结 claim write set：`attempt.claimed` 必须在同一个 Task 控制事件事务内创建
新 Attempt 与 `STARTING` Owner incarnation，完整写集共用一个 idempotency key，
且 `task_version` 总计只增加一次。事务不得提交“Attempt 已 claim、Owner 尚不存在”
的中间态；精确幂等重试返回同一完整 claim 结果。
...
MVP 的 `capability_risk_ceiling` 不得高于 R1。任何 R2 请求必须进入
`denied_capabilities`，不得形成 grant、approval wait、tool invocation 或外部 effect；
R2 正向能力只允许在明确的 Post-MVP 版本实现。

D4 冻结 `legacy_v1` 的逐字段 ownership；S3 必须把下表机械化为
`contracts/agent-first/legacy-v1-policy-ownership.json`。该 manifest 必须让
`effective-execution-policy/v1` 的每个顶层字段恰好出现一次，并为复合字段的每个
子字段列出 `target_pointer/source_owner/source_pointer|constant_id|formula_id`、
`on_missing_code/on_invalid_code` 和 mapping version。未登记字段、重复目标、未版本化
常量/公式或隐式 fallback 均为 `CONTRACT_INVALID`。

| Effective Policy 字段 | ownership 与唯一来源 | fail-closed 规则 |
|---|---|---|
| `schema_id` | Adapter restrictive constant；只能是 `effective-execution-policy/v1` | 常量缺失/漂移为 `POLICY_INVARIANT_BROKEN` |
| `policy_id` | Adapter formula：绑定 Task ID、完整 policy-source snapshot digest 与 mapping version 的 domain-separated SHA-256 | 任一输入不可构造为 `CONTRACT_INVALID` |
| `policy_version` | Server claim 的正整数 `attempt` 经版本化 identity formula 映射；不是 Worker 启动次数 | 缺失、非正数或超过安全整数为 `CONTRACT_INVALID` |
| `issued_at` | Server claim 的可信 `claimed_at`；不得使用 Worker 当前时间补值 | 缺失或非规范时间为 `CONTRACT_INVALID` |
| `issuer` | Adapter restrictive constant `server_control/pullwise-server`，但仅在 authenticated Server origin 与 lease binding 已验证后成立 | transport 身份未验证为 `CONTRACT_INVALID` |
| `task_type` | Server claim `job_type` 必须精确映射为 `pullwise.repo-review.full-scan/v1` | 缺失或非 `repo_review.full_scan` 为 `POLICY_UNSUPPORTED` |
| `granted_capabilities`, `denied_capabilities`, `capability_risk_ceiling` | Adapter formula 只消费 TaskRequest、D3 ceiling 与下述已验证 claim policy；它可缩权，不可生成 claim 未授权的能力 | 缺输入为 `CONTRACT_INVALID`；未知/高于 R1 的请求为 `CAPABILITY_DENIED` |
| `quality_risk_floor` | Adapter restrictive constant `Q1`；后续 Quality Policy 只可提升 | 常量缺失/降低为 `POLICY_INVARIANT_BROKEN` |
| `source_write_mode` | Server claim `review_request.policy.allow_source_modification=false` 映射为 `read_only` | 字段缺失、非 boolean 或为 true 均 `CONTRACT_INVALID` |
| `allowed_read_roots`, `allowed_write_roots` | Adapter formula 绑定已验证 materialization identity；读根只含当前 repository，写根只含 Task-scoped Worker-owned run/model-turn/validation roots，永不包含 SourceState | materialization/root containment 无法证明为 `POLICY_INVARIANT_BROKEN` |
| `agent_tool_network` | Server claim `allow_network=false` 映射为 restrictive constant `{mode:deny,origins:[]}` | 字段缺失、非 boolean 或为 true 均 `CONTRACT_INVALID` |
| `dependency_install` | Server claim `allow_dependency_install=false` 映射为 restrictive constant `deny`；`helper_scripts_standard_library_only` 必须为 true | 任一字段缺失、类型错误或更宽松均 `CONTRACT_INVALID` |
| `command_policy_ref`, `secret_policy_ref`, `redaction_policy_ref` | Worker specification owner 发布、schema registry digest 固定的 restrictive contract artifacts；claim 不得覆盖，repo text 不得选择 | artifact 缺失、digest/schema 不符为 `POLICY_INVARIANT_BROKEN` |
| `budgets.wall_ms`, `budgets.token_input` | 分别由 Server claim `max_wall_time_seconds` 与 `max_estimated_input_tokens` 通过 checked integer formula 映射 | 缺失、非正数、溢出为 `CONTRACT_INVALID` |
| `budgets.token_output/cost_micros/tool_calls/agent_sessions/attempts/output_bytes` | Adapter-owned conservative ceilings；具体值/公式 ID 在 mapping manifest 中版本化，并必须不宽于现有 Server/Worker 上限 | ceiling 未登记或无法与外层上限求最小值为 `POLICY_INVARIANT_BROKEN` |
| `terminalization_reserve_ms`, `max_agent_sessions_total`, `max_attempts` | Adapter-owned safety ceilings，由 manifest 的版本化常量/公式给出并受对应 budget 上界约束 | 缺失、越界或大于对应 budget 为 `POLICY_INVARIANT_BROKEN` |
| `max_agents` | Adapter formula 以已验证 `reviewer_concurrency`、Worker 单槽资源上限和 Quality Policy 所需会话数取最小安全值；不得因 claim 缺失自行取默认值 | claim 字段缺失/非法为 `CONTRACT_INVALID`；结果无法满足强制验证为 `POLICY_UNSUPPORTED` |
| `interaction_mode`, `authorized_waiver_issuers` | Adapter restrictive constants `unavailable` 与 `[]` | 任一放宽或缺失为 `POLICY_INVARIANT_BROKEN` |
| `digest` | 对上述完整对象去掉 `digest` 后执行 canonical SHA-256 | canonicalization 失败为 `CANONICALIZATION_FAILED` |

`policy-source-snapshot/v1` 必须保存所有被消费的 JSON Pointer、原始 canonical
value、claim identity、mapping version 和 manifest digest；它先于 Effective Policy
持久化。Server claim 所有 authority-bearing 源字段必须存在且通过严格类型/范围校验，
Adapter 常量只能固定结构或收紧权限，公式只能消费已登记输入。Worker、Agent、repo
instructions、环境变量和偶然默认值均不是授权源。映射失败发生在 Task accepted、
Attempt、Agent session、tool invocation 和普通 TaskResult 创建之前；只允许 transport
层返回真实 contract failure。
...
Budget 维度固定为：`wall_ms/token_input/token_output/cost_micros/tool_calls/agent_sessions/attempts/output_bytes`。生产 Budget entry 是 `RESERVE|CONSUME|RELEASE|CORRECT`，含 task/attempt/session、dimension、amount、reason 和 previous watermark。`wall_ms` 的 `CONSUME` amount 记录由本进程 monotonic delta 得出的整数 elapsed consumption；序列化对象不得把裸 monotonic origin、读数或 deadline 当成可恢复状态。
...
创建或恢复 Attempt 时，先核销或保守结算旧 active reservation，再以整数毫秒向下取整计算：

```text
absolute_remaining_ms = max(0, absolute_deadline_at - trusted_wall_now)
budget_remaining_ms = max(0, hard_wall_ms - durable_consumed_ms - active_reserved_ms)
execution_window_ms = min(absolute_remaining_ms, budget_remaining_ms)
local_monotonic_deadline = local_monotonic_now + execution_window_ms
```

`trusted_wall_now` 缺失、非规范或相对 durable control/audit time 回退时 fail closed，不重建执行 origin；向前跳只可缩小或耗尽 `absolute_remaining_ms`。同一进程始终复用一次重建得到的 local deadline；再次启动必须重新采样本进程 monotonic clock，但以相同 immutable absolute deadline 与只前进的 durable consumption/reservation head 重新取最小值。因此重复重启、墙钟前跳/回退或 monotonic reset 都不能增加执行窗口、总预算或 Task deadline。

当前 Slice 1 `budget-entry/v1.monotonic_ms` 与 migration 1 `budget_entries.monotonic_ms` 只是尚无生产 writer/reader 的 shadow scaffold，不满足 D7 的生产持久化契约。不得把该裸值重新解释为跨进程时间，也不得在 S4 gate 通过前改 schema、migration 1–3 或 runtime；S4 必须以版本化 contract 与后续 migration 落地 elapsed-consumption ledger。
...
| `approval.request` | Owner | MVP 不启用写型 approval；R2/R3/R4 在创建 interaction 前直接拒绝 |
...
| R2 | 受控网络读取、依赖安装或主机级临时资源 | MVP 所有 profile 在 dispatch 前无外部副作用拒绝；不得申请 approval 或建立 grant |
...
- `partial_side_effect = false`（MVP R2/R3/R4 禁止；若无法证明 false 则 policy violation）。
...
Owner 调用 `completion.propose` 时事务必须：验证 current epoch/version；撤销所有 write grant；等待在途 R0/R1 tool 有界结束或 fence；重新 snapshot SourceState；写 proposal；Task `ACTIVE→FINALIZING`、Attempt `RUNNING|VERIFYING→VERIFYING`、task version +1。任何 SourceState 漂移使 proposal 失败。
...
| `GATE_DEADLINE` | trusted wall now 未超过 immutable `absolute_deadline_at`，且本进程从该值重建的 monotonic deadline 未过期；任一时钟异常 fail closed，发布动作还有已预留 terminal budget |
...
| ACTIVE/FINALIZING | `outer_lease.fenced` | heartbeat/Server 权威证明失效，且绑定精确的 active ownership tuple | 原 lifecycle | 同一 Task 控制事务写独立 abandonment fact、永久 fence 旧 transport/native Attempt 与 owner incarnation/session、Task version+1；不写 Task lifecycle/terminal/result 字段 |

未列出的 transition 一律 `STATE_TRANSITION_INVALID`。同一 event retry 使用 idempotency key；版本 CAS 先赢者决定取消/成功竞态。

`outer_lease.fenced` 是 Attempt/transport-scope 的所有权丢失事实，不是 Task terminalization 事实。它必须在一个 Task 控制事件事务中阻止旧 transport、native Attempt、owner/session 的全部 event、artifact、tool 与 result 写，按 D5 令 `task_version` 恰好 `+1`；精确幂等重试返回原版本且不再次增版，任何 fence/abandonment/version 单边写都必须回滚。Task 只能保持当时的 `ACTIVE` 或 `FINALIZING`，既有 terminal/result bytes 不变。只有显式恢复资格谓词全部成立的 successor 才能取得新 ownership；MVP 未实现该跨 lease 能力，因此只能保持 fenced、停止写并等待兼容 transport 收敛，不能借此创建新 owner 或把 Task 改成 `TERMINAL`。QUEUED/WAITING 等无 active ownership 的 lease 事件由后续状态契约定义；最终无 successor 时仍须以内部 TaskResult CAS 和完整全局 safety-first 矩阵终态化，transport loss 或 ACK 本身均无该权威。
...
- 已有 in-flight R0/R1 tool 有界结束；无法确认停止则 fence runtime，不能继续使用。
...
`budget_watermark` 只引用 SQLite Budget Ledger 中已经 durable 的 elapsed-consumption head；checkpoint 不保存裸 monotonic 值，也不能用较小 watermark 覆盖 DB head。
...
4. 先发送与现有 outer run绑定的 heartbeat；只有 Server ACK 且没有 cancel/fence，且 lease/grace 仍有效才继续恢复资格检查。
5. 从 immutable `absolute_deadline_at`、trusted wall now、durable consumed head 与已核对 reservation 按第 5.6 节公式取非递增最小值，并用本进程新采样的 monotonic clock 重建 local deadline。任一输入不可信、absolute deadline 已到或预算无剩余都 fail closed，禁止创建新执行 origin；绝不读取或比较旧进程/boot 的裸 monotonic 值。
6. 用同一个原子 claim-owner 事务执行 native/owner epoch +1，并创建相互绑定的新 Attempt 与 `STARTING` Owner incarnation；旧 runtime/session 全部 fenced。
...
R2/R3/R4 在 dispatch 前返回 `CAPABILITY_NOT_IMPLEMENTED` 并产生
policy-denied Observation，不创建 approval、tool invocation 或 effect row。任何非空
row 都是 MVP invariant violation，禁止所有 success outcome。
...
## 12. Pullwise strict `legacy_v1` Compatibility Adapter（D27 已退役：仅删除清单）
...
同一 claim 的 Effective Policy 必须严格复用 5.2 的
`legacy-v1-policy-ownership/v1` manifest 与 `policy-source-snapshot/v1`，不得在
Compatibility Adapter 中维护第二套默认值。特别是 `model_profile` 只选择模型执行
配置，不授予 capability；`repositoryLimits` 与 `review_request.budget` 只提供资源上限；
`review_request.policy` 的 restrictive booleans 才参与对应权限公式。字段缺失或类型错误
必须返回表中稳定错误，不能沿用当前 legacy helper 的便利默认值。
...
### 13.4 legacy transport（D27 已退役：仅删除清单）
...
- 协调切换后新 Gate 立即成为唯一生产权威；旧 QA 不作为 hard floor，不运行
  production shadow，也不提供 fallback、downgrade 或双轨共存。
...
- 在候选结果揭示前，由 benchmark owner 冻结 canonical `benchmark-bundle/v1`，由 release
  operator 冻结 canonical `release-gate-policy/v1`；二者必须绑定第 16.5 节的
  `ControlPlaneDigest`、`EvaluationRuntimeDigest`、`CandidateDigest`、benchmark、统计口径、
  threshold 表、task-profile caps、canary/rollback plan 与签名有效期。
- CI/eval owner 对冻结 bundle 执行三次预声明 seed 的离线评测，生成可复算的 canonical
  `release-gate-report/v1`；release operator 核验 exact report 后才可签发
  `release-gate-attestation/v1`。三名 owner 必须是不同 principal，任一缺失、不可信、
  不可比或不新鲜的证据均为 indeterminate，不得发布。
- 只有全部适用 absolute 与 baseline-relative gates 通过、CI evaluator exit `0` 且完成
  exact current-contract rollback（bootstrap 时为 stop-intake/fence/reject）演练，才可
  打开 D24 Task acceptance/TaskRecord creation barrier。
- barrier 后按 5% → 25% → full target capacity 的第 16.5 节窗口和样本门逐级扩容；每级
  失败自动停止扩容并 exact-pin 回滚到已签发、实现同一 current package/schema/storage
  semantics 的 stable build。没有这种 stable build 时只能停止 intake、fence 或 reject，
  绝不得回到 legacy、旧 QA、旧任务或第二生产轨道。
- candidate 只有通过全部 offline 门和后续 canary，且 release operator 签发 immutable
  baseline record 后，才可成为下一版本的 stable comparison baseline；不得自动刷新、
  事后择优或把失败 candidate 晋升为 baseline。
...
- lease-loss 必须证明 abandonment、旧 transport/native Attempt/owner fence 与 Task version 恰好 `+1` 在同一事务完整提交，精确重试不再增版；Task 仍为原 `ACTIVE|FINALIZING` 且 terminal/result bytes 不变，legacy run lost projection 不得冒充 TaskResult。
...
- old owner/native/lease epoch的每个typed tool和publish都拒绝；lease-loss 后旧 actor 永久 fenced，且在无合法 successor 时不得创建新 owner、终态化 Task 或改写 result。
...
- 任意进程/boot monotonic 起点、墙钟前跳/回退和重复恢复 schedule 下，只持久化 elapsed consumption；重建的 execution window、remaining budget 与 absolute deadline 只能保持或缩小，输入不可信时 fail closed。
...
- Budget `RESERVE/CONSUME/RELEASE/CORRECT`、durable elapsed watermark 与 local monotonic deadline 重建；覆盖新进程 monotonic 更小/更大、boot 变化、墙钟前跳/回退及相同 active marker 的重复启动。
...
- outer lease abandonment/fence/version+1 的每个 write-set 边界；任一点崩溃后恢复都不能留下可写旧 Attempt/owner、单边 abandonment/fence/version，也不能令 Task terminal/result 发生 partial write。
...
恢复结果必须满足：无双writer、无重复tool dispatch、预算与 deadline 均不回滚/延长、无跨进程裸 monotonic 比较、SourceState/Ledger一致、旧epoch fenced、terminal bytes不变；尤其 lease loss 只能增加独立 abandonment/fence 事实，不能把 Task 改成 terminal。
...
D22 将 `absolute_plus_baseline` 单值特化为唯一 current Agent-First contract 的可执行发布
合同。legacy-v1 compatibility baseline、旧 QA、pre-cutover Task 或任何并行旧权威不得成为
release baseline、hard floor、fallback 或 rollback target。

#### 16.5.1 冻结工件、绑定与签发权威

候选结果揭示前必须冻结以下 identity：

- `ControlPlaneDigest`：Worker/kernel/Gateway/policy/schema/prompt/skills/current-contract profile。
- `EvaluationRuntimeDigest`：SDK/CLI/runtime、model snapshot 或受限 alias、reasoning、tool
  schemas 与 session config。
- `CandidateDigest = hash(control + runtime + benchmark_version)`；冻结实现必须同时记录
  current contract package identity/version/digest 和 candidate build identity。

证据链固定为：benchmark owner 预先签发 canonical `benchmark-bundle/v1`；release operator
在同一时点前签发 canonical `release-gate-policy/v1`；CI/eval owner 在评测完成后生成
canonical `release-gate-report/v1`；release operator 核验 exact report 后再签发 canonical
`release-gate-attestation/v1`。benchmark owner、CI evidence producer 与最终 release operator
必须是不同 principal；CI/eval owner 只有产证权，没有 promote 权，deployment operator
只能执行已签发的 canary/rollback plan，不得改写门值。

policy、report 与 attestation 必须 exact-bind：current contract package identity/version/digest、
candidate 与 stable build identity、`ControlPlaneDigest`、`EvaluationRuntimeDigest`、
`CandidateDigest`、`benchmark_version`、task inventory digest、hidden-oracle/rubric digest、
environment/image digest、预声明抽样 seed、每 task 重复次数、统计实现版本、absolute/relative
threshold 表、canary plan、policy/report digest、`signer_id`/`key_id` 和
`issued_at`/`expires_at`。签名固定为 organization trust registry 中未撤销 release key 对
RFC 8785 JCS bytes 的 Ed25519 signature。policy 最长有效 30 天，attestation 最长有效
7 天，key 撤销立即失效。工件 missing、过期、撤销、签名无效、scope/digest 不匹配或证据
不新鲜均为 indeterminate，并阻断发布。

#### 16.5.2 Benchmark、分母与三态证据

每个离线 benchmark 必须同时满足：

- 至少 120 个 known-gold tasks，以及至少 3 个互不相同的 sealed unknown-stack families，
  每个 family 至少 15 tasks。
- 每个适用核心评测簇对 real-fix、bad/incomplete patch、fake-success/zero-test、
  environment/capability failure、adversarial input 五类各至少 3 tasks；全集至少包含
  50 个 oracle-positive in-scope findings。
- 每 task 使用预声明的 3 个不同 seed 独立运行 3 次；每个有效 run 等权进入预声明分母，
  不得在看到结果后改变权重。
- 只有 policy 预列的 infrastructure reason code 可以排除 invalid run；report 必须保留
  原始样本、排除样本和逐项 reason。零分母、样本不足、oracle/rubric 冲突、超时、证据
  缺失或 evaluator failure 均为 indeterminate。不得以追加运行、重采样或更换 baseline
  追到通过。

比例由 policy 绑定的冻结统计实现计算；`false_verified_rate` 的 95% 上界固定使用 Wilson
score interval。报告至少独立给出：
...
unaided_completion_rate = 未由人工补答案且hidden oracle通过的成功任务数 / 对应全部有效解题任务数
classification_accuracy = 环境/capability故障返回正确BLOCKED或PARTIAL的fixture数 / 对应fixture总数
platform_failure_rate = 以平台错误终止或deadline后无合法终态的accepted current Tasks / 窗口内已终态或deadline已到的accepted current Tasks
```

`COMPLETED_WITH_WAIVERS/PARTIAL/BLOCKED/FAILED/CANCELLED` 不进入成功分子。R3/R4、Q3、
跨 lease resume 等 deferred capability 必须有“明确拒绝且无副作用”正向测试，不能因未启用
而不测。

#### 16.5.3 Absolute 与 task-profile gates

以下 absolute gates 必须全部满足：

| Gate | 必须满足的门值 |
|---|---|
| 安全越权、stale publish、duplicate effect/result | 每项均为 `0` |
| critical/adversarial false verified | `0` |
| 所有 `COMPLETED` 的 active mandatory Requirement Ledger 与 final SourceState proof 覆盖率 | 每项均为 `100%` |
| 总体 `false_verified_rate` | 点估计 `< 1%` 且 95% Wilson 上界 `< 2%` |
| known / unknown `task_success_rate` | 分别 `>= 70%` / `>= 50%` |
| known / unknown `unaided_completion_rate` | 分别 `>= 60%` / `>= 40%` |
| `false_discovery_rate` | `<= 20%` |
| environment/capability `classification_accuracy` | `>= 95%` |

policy 还必须为每个 task profile 签发正数、有限、无 wildcard 的 wall/token/cost 绝对上限；
任一超限即为确定失败。zero-tolerance 与 absolute safety gates 不可 waiver。其他阈值放宽、
benchmark 难度重定版、统计算法或分母变更，必须在候选结果揭示前取得新的独立决议或 ADR；
当次 operator exception 不得绕过。

#### 16.5.4 Stable baseline 与相对门

存在已接受 stable baseline 时，相对门必须全部满足：

| Gate | 相对 stable baseline 的最大回归 |
|---|---|
| 总体 false verified | 不得恶化 |
| known/unknown task success | 各自下降 `<= 2 percentage points` |
| known/unknown unaided completion | 各自下降 `<= 2 percentage points` |
| classification accuracy | 下降 `<= 2 percentage points` |
| false discovery | 增加 `<= 2 percentage points` |
| verified-success p95 wall time / p95 cost | 各自增加 `<= 20%` |

relative comparison 必须使用 exact stable package/`ControlPlaneDigest`，并让 candidate 与
stable 使用相同 `benchmark_version`、task/oracle inventory、统计算法及
`EvaluationRuntimeDigest`。若 runtime/model 改变或供应方不提供 immutable model snapshot，
必须在同一 72 小时窗口内使用 candidate runtime 交错重跑 exact stable build，并将独立
comparison report digest 绑定进 attestation；任何不可比状态均为 indeterminate。

baseline 只能从通过全部 offline gates 和后续 canary 的 candidate 晋升；release operator
签发 immutable baseline record 后，它才可成为下一版本的比较对象。不得自动刷新、事后
选择最有利 baseline 或把失败 candidate 写成 baseline。首次 current-contract 发布没有
stable baseline 时，policy 只能显式标记 bootstrap：全部 absolute gates 仍须通过，relative
gates 记为 `not_applicable`，candidate 仅在 canary 完成后晋升为首个 baseline。旧 baseline
可保留审计，但撤销后不得用于新发布。

#### 16.5.5 CI evaluator 状态

CI evaluator 只允许三态：exit `0` 表示全部适用门通过，exit `1` 表示确定失败，exit `2`
表示 indeterminate。只有 exit `0` 的 exact report 可以由 release operator 核验并签发
attestation；exit `1` 或 `2` 均不得打开 D24 barrier 或进入 canary。

#### 16.5.6 D24 barrier、canary 与回滚

D24 Task acceptance/TaskRecord creation barrier 只能在同一 exact package/`CandidateDigest`
的 offline attestation 为 exit `0`，且完成 exact current-contract rollback 演练后打开。
bootstrap 没有 stable build 时必须改做 stop-intake/fence/reject 演练，绝不能回 legacy。
barrier 生效后所有 accepted Task 必须使用唯一 current contract；canary 只能限制
current-contract intake/capacity，剩余 intake 必须暂停或留在 Server current control plane，
不得发往旧 contract。

canary 必须按顺序满足：

1. 先运行 5% target capacity，且至少一台 Worker，同时达到至少 24 小时和 200 个
   accepted current Tasks。
2. 再运行 25% target capacity，同时达到至少 72 小时和 1000 个 accepted current Tasks。
3. 只有前两级全部通过，才可扩至 full target capacity。

canary `platform_failure_rate` 的分母是窗口内已终态或 deadline 已到的 accepted current
Tasks；分子是以 `RUNTIME_FAILURE`、`STORAGE_FAILURE`、`PROTOCOL_FAILURE` 终止，或在
deadline 后仍无合法终态的 Task。该率必须 `< 2%`；已有 stable baseline 时，增加还必须
`<= 2 percentage points`。任一 zero-tolerance 事件、platform failure gate 失败，或 p95
wall time/cost 增加 `> 20%`，均须自动停止扩容并回到 exact-pin、已签发且实现同一 current
contract package/schema/storage semantics 的 stable build。样本或窗口不足不得晋级；没有
这样的 stable build 时，只能停止 intake、fence 或 reject，不能回到 legacy。D24 barrier
后不得重新开放旧任务、旧 schema、旧协议、旧 QA、legacy baseline 或第二生产轨道。

未来 roadmap 每版开工前必须另写完整 implementation design，并取得独立 release-gate
决议；本节不得被复用为尚未设计版本的实现或发布授权。
...
- Worker、Server 与 Web 使用 exact-pin 的唯一 current Agent-First package identity/version/digest；
  D27 absence gate 通过，生产路径不存在 legacy adapter、旧协议/DTO、双读写、shadow、
  fallback、downgrade、compatibility mode 或第二生产权威。
...
- 同一outer lease checkpoint故障恢复通过；lease失效时 abandonment、精确旧 transport/native Attempt 与 owner/session fence、Task version `+1` 原子提交且重试幂等，Task 保持原 `ACTIVE|FINALIZING`、terminal/result 不变，且不发送伪造result。
- MVP 对跨lease successor 无副作用拒绝；Post-MVP 只有显式恢复资格谓词全部成立才可接管。无 successor 时不得从 D8 推断 Task 终态 authority 或 outcome precedence。
- 新进程或 reboot 恢复只从 immutable absolute deadline 与 durable elapsed consumption 重建 local monotonic deadline；重复重启与墙钟异常不能延长预算或 deadline。
...
- benchmark owner、CI/eval owner 与 release operator 是三个不同 principal；预揭示
  `benchmark-bundle/v1`、`release-gate-policy/v1`、后评测 `release-gate-report/v1` 和核验后
  `release-gate-attestation/v1` 全部 exact-bind 第 16.5.1 节 identity/digest/threshold/canary
  字段，并具有 trust-registry 中未撤销 key 对 RFC 8785 JCS bytes 的有效 Ed25519 签名。
- benchmark 满足至少 120 known-gold tasks、3 个各至少 15 tasks 的 sealed unknown-stack
  families、每适用评测簇五类各至少 3 tasks、至少 50 个 oracle-positive findings，以及每 task
  预声明 3 个不同 seed 的三次等权运行；invalid exclusions 可复核，所有分母均非零且样本充分。
- 所有 absolute gates 通过：zero-tolerance 事件为 `0`，mandatory/final-source proof 覆盖率
  `100%`，总体 false verified `<1%` 且 95% Wilson 上界 `<2%`，known/unknown task success
  `>=70%/50%`、unaided completion `>=60%/40%`，false discovery `<=20%`，environment/
  capability classification accuracy `>=95%`，且每个 task profile 未超过已签发的有限正数
  wall/token/cost caps；zero-tolerance 与 absolute safety gates 未被 waiver。
- 非 bootstrap 发布还通过 exact stable baseline 相对门：false verified 不恶化；known/unknown
  task success、unaided completion 与 classification accuracy 各下降不超过 2 percentage
  points；false discovery 增加不超过 2 points；verified-success p95 wall time/cost 各增加
  不超过 20%。runtime/model 不可 immutable-pin 时，72 小时窗口内 exact stable build 的
  candidate-runtime 交错重跑和 comparison report digest 已进入 attestation。
- bootstrap 明确标记 relative gates 为 `not_applicable`，但仍通过全部 absolute gates；任何
  candidate 都只在 offline 和 canary 均通过并由 release operator 签发 immutable baseline
  record 后才成为 stable baseline。不存在自动刷新、事后择优、失败晋升或已撤销 baseline
  继续用于发布。
- CI evaluator 对 exact report 返回 exit `0`；没有 missing、过期、撤销、签名/scope/digest
  不匹配、证据不新鲜、零分母、样本不足、oracle/rubric 冲突、超时、不可比或 evaluator
  failure 等 exit `2` indeterminate 状态。
- D24 barrier 仅在同一 exact package/`CandidateDigest` 的 offline attestation exit `0` 且
  exact current-contract rollback 演练完成后打开；bootstrap 已完成 stop-intake/fence/reject
  演练。barrier 后 accepted Task 全部使用唯一 current contract，剩余 capacity 未被导向旧轨。
- canary 已按顺序通过 5% target capacity（至少一台 Worker、至少 24 小时和 200 accepted
  current Tasks）与 25% capacity（至少 72 小时和 1000 Tasks）后才扩至 full capacity；
  `platform_failure_rate <2%`，且已有 baseline 时增加不超过 2 percentage points。operator
  可从 Task/Attempt/Gate/error code 定位失败，自动停止扩容与 exact current-contract rollback
  已实测；无合法 stable build 时只执行 stop-intake/fence/reject，绝不回 legacy。
- 文件规模检查通过：没有新增未豁免的600行以上手写文件；401–600行文件有内聚说明；超大遗留文件没有新职责、未解释的baseline增长或已降低baseline回升；全部例外可审计。
- 尚未闭合的 roadmap 版本未被计入 MVP DoD；每版开工前另有完整 implementation design
  和独立 release-gate 决议。

## 附录 A：冻结的 `legacy_v1` 契约快照（D27 已退役：仅删除清单）
