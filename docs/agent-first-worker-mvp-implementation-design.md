...
- opaque ID 格式为 `<prefix>_<32 lowercase hex>`，由 128-bit CSPRNG 产生。固定前缀见各 schema。ID 最长 80 字符。
...
不得用空字符串、缺字段或裸 `null` 表示未知；唯一例外是 TaskResult 明确定义的 `change_set_ref`。
...
| `task_id` | `task_<32hex>`；Pullwise Adapter 可使用稳定映射后的 `scan_id` |
...
`effective-execution-policy/v1` 至少含：
...
- `network {mode:deny|allowlist,hosts[]}`；
...
请求是能力愿望，不是 grant。Task Owner、repo instructions、tools 和 Adapter 只能请求更低/相同权限。Pullwise full-scan profile 必须拒绝 source modification、dependency install、network、非标准库 helper；这些值来自 Server claim，缺失时 job fail closed，而非本地默认。
...
`waiver-event/v1` 必须绑定 `task_id/requirement_id/ledger_digest/policy_version/issuer/key_id/reason/issued_at/expires_at/signature`。MVP Pullwise legacy profile 的 `authorized_waiver_issuers=[]`，因此所有 waiver 请求拒绝；local/eval 只有控制面 keyring 验证成功时可接受。Agent 自述“用户同意”不是授权。
...
- `include = tracked_and_declared_delivery` 或 Pullwise 的 `all_repository_regular_files`。
...
- actor union `{kind:task_owner|quality_verifier|legacy_domain_reviewer|worker_control, id, session_id|null}`。
- `tool_id/tool_version/tool_invocation_id/idempotency_key/input_digest`。
- `status = succeeded|failed|timed_out|cancelled|policy_denied|infrastructure_error`。
...
### 8.6 EvidenceClosureManifest

Closure 必须包含从 TaskResult 可达的：request、policy、charter、Ledger snapshot、waiver events、proposal、source/execution manifests、change set、pre/final ObservationManifest、每条 Observation raw artifact、verifier input/work/attestation、交付 artifact/report、empty Effect Ledger、budget summary、GateDecision。

算法固定：

1. 从声明的 roots 深度优先解析 ContentRef；schema registry 给出每种对象的 ref-bearing JSON pointers。
2. 验证 bytes/size/hash/schema；拒绝 alias、循环、未知 schema、跨 Task ref。
3. 对完全相同 ContentRef 去重；同 artifact ID 不同 digest 冲突。
4. 排除 closure manifest 自身和 TaskResult 自身。
5. entries 按 `(content_schema_id,artifact_id,sha256)` 升序。
6. `closure_digest = sha256(CanonicalJSON(entries))`；manifest bytes 自身另有 ContentRef SHA。
...
Gate 前先冻结 `gate-input-snapshot/v1`，绑定当时的：Task version/lifecycle/desired state/epoch、lease、deadline/budget、request/policy/Ledger/proposal/source/execution/manifest/attestations/effects/closure。Gate 只读该 snapshot，不边读边变。
...
| `GATE_EVIDENCE_CLOSURE` | 所有 transitive refs bytes/hash/size/schema 一致 |
| `GATE_BUDGET` | 没有超额/负 reservation/未结算 Agent session |
| `GATE_SECRET_SCAN` | 普通 artifacts、result、debug fragment 均通过 secret policy |

Gate 输出 `gate-decision/v1`：每个 predicate 的 boolean、stable failure code、repairability、evidence refs、input digest。相同 input digest 必须返回逐字节相同 decision。
...
MVP Effect Ledger 必为空，因此 Terminalization Gate 只需证明：
...
Terminalization 不能生成 PASS、伪造缺失 manifest 或把 `UNVERIFIABLE` 改写为成功。
...
| 任意非终态 | `cancel.finalized` | no in-flight tool/effects empty | TERMINAL | CANCELLED result、Attempt CANCELLED |
| 任意非终态 | `outer_lease.fenced` | heartbeat/Server权威证明失效 | TERMINAL | abandonment record、Attempt FENCED、无 Worker TaskResult |
...
`attempt-record/v1` 必填：`attempt_id/task_id/native_epoch/transport binding/state/state_version/predecessor_checkpoint_generation/owner_session_id/lease_acquired_at/started_at/ended_at/termination_reason/budget_reservation_id`。
...
3. durable commit 新 checkpoint generation。
4. 同一事务将 Attempt `SUSPENDING→SUSPENDED`、Task `ACTIVE→WAITING_*`、记录 interaction。
...
- `task_version/native_epoch/attempt_id/owner_epoch`。
...
Commit 协议：所有 referenced bytes 先入 CAS并验证 → 写 manifest CAS → SQLite `BEGIN IMMEDIATE` 检查 `generation=current+1` 和 previous hash → 插 checkpoint index → 更新 Task pointer/version → commit。完全相同 generation/hash 重试幂等；跳代、分叉、同 generation 不同 hash 拒绝。
...
- `attempt_id/native_epoch/owner_id/owner_epoch`。
- `request_ref/policy_ref/charter_ref/requirement_ledger_ref`。
...
- `diagnostics.worker_debug_fragment`。
- `created_at/terminal_at`。

TaskResult 不包含自己的 digest/ref。`result_publications` 保存 TaskResult ContentRef/digest。`TaskResultCore` 是按 JSON Pointer 只删除 `/diagnostics/worker_debug_fragment` 后的对象；保留 `diagnostics:{}` 或其中其他字段，不能删除父对象。`task_result_core_digest` 在 terminal fragment 前计算，完整 result digest 在 fragment descriptor 写入后计算。

### 11.3 outcome-discriminated oneOf

| Outcome | 必要条件 | 特殊字段 |
|---|---|---|
| `COMPLETED` | 全部 mandatory PASS；Success Gate pass | analysis/report 可 `change_set_ref=null`；change task 可非空 |
| `NO_CHANGE_NEEDED` | `intent_kind=change`；全 mandatory PASS；original=final；运行 Observation 证明已满足 | `change_set_ref=null`，reason=`ALREADY_SATISFIED` |
| `COMPLETED_WITH_WAIVERS` | 未 waiver mandatory PASS；其余有当前有效授权 | 不计完整成功；列出每项原 verdict/waiver |
| `PARTIAL` | 存在有用且安全的 deliverable；至少一个 mandatory FAIL/UNVERIFIABLE | 必须列 `delivered_scope/gaps/residual_risks` |
| `BLOCKED` | 缺输入、权限、capability或必要环境且无可安全 partial deliverable | 必须列 blocker 与解除条件 |
| `FAILED` | 不可恢复的 runtime/storage/protocol/quality failure，且无可安全 deliverable | 必须列 failure evidence；不能用于可请求输入的情况 |
| `CANCELLED` | desired CANCEL 先线性化；effects empty | 不允许 requirement PASS 推断为成功 |
...
- `task_id` 使用 Server `scan_id` 的稳定合法映射；原值保存在 transport binding。
...
- constraints 逐项从 `repositoryLimits/model_profile/review_request.policy/review_request.budget/output_language` ingest。
...
- HTTP 200 后记录 ACK。
- 在明确收到 response 前连接中断时，不重发同 sequence；标记 delivery unknown，下一事件使用更大 sequence。Server允许 gap，从而避免把非幂等 event replay当成功。
...
```json
{
  ...
  "reviewWorkerProtocol": {
    ...
    "progress_final": {"overall_percent": 100.0, "current_phase": "submit_result_envelope", "status": "completed", "message": "terminal progress", "steps": []},
    "summary": {"overall_risk": "unknown", "result_status": "complete", "finding_counts": {}, "coverage": {}, "top_findings": []},
...
- `task_result_core_digest` AvailabilityRef；只有terminal必须available，其他为not_applicable。
- `source_state_id`只记录digest，不嵌入source。
- `file_manifest_ref/redaction_report_ref/status/reason_code`。

`fragment_id=frag_<sha256(identity + capture_kind + snapshot_seq + file entries)>`；同capture重试bytes必须相同。terminal fragment在TaskResultCore之后、完整TaskResult之前冻结，避免hash循环。
...
| 类别 | 必备codes |
|---|---|
| Contract | `SCHEMA_INVALID`, `SCHEMA_VERSION_UNSUPPORTED`, `CANONICALIZATION_FAILED`, `SPEC_GAP` |
| Identity/fence | `TASK_VERSION_STALE`, `OWNER_EPOCH_STALE`, `NATIVE_EPOCH_STALE`, `LEASE_INVALID`, `TRANSPORT_IDENTITY_MISMATCH` |
| State | `STATE_TRANSITION_INVALID`, `TASK_ALREADY_TERMINAL`, `IDEMPOTENCY_CONFLICT` |
| Policy | `CAPABILITY_DENIED`, `CAPABILITY_NOT_IMPLEMENTED`, `ROLE_NOT_ENABLED`, `SOURCE_MUTATION_FORBIDDEN`, `INTERACTION_UNAVAILABLE`, `POLICY_INVARIANT_BROKEN` |
...
| Runtime/transport | `RUNTIME_UNHEALTHY`, `TOOL_TIMEOUT`, `EVENT_DELIVERY_UNKNOWN`, `TRANSPORT_CONSISTENCY_FAILED`, `RESULT_CONFLICT`, `JOB_CANCELLATION_AUTHORITATIVE` |
| Debug | `DEBUG_REDACTION_FAILED`, `DEBUG_LIMIT_EXCEEDED`, `DEBUG_UPLOAD_FAILED`, `DEBUG_UNAVAILABLE` |
...
完成证据：代码地图、contract fixture hashes、当前baseline test结果；零生产行为变化。
...
每个Slice必须同时更新schema/fixture/test/metrics/runbook；只有文档或只有happy-path code不算完成。
...
本附录以2026-07-16只读工作树为准。Server当时HEAD为`b271383fc34592676307d700666419703238d164`，Web HEAD为`0af3088a86a3474b6f8b937958137db438098d48`。实现若看到hash变化，必须先运行compatibility diff和fixtures，不能把旧hash当现行事实。

| 权威文件 | SHA-256 | 关键位置 |
|---|---|---|
| `pullwise-server/AGENTS.md` | `7d6b6b84b5140b160146410005b01af5c4c59825f7f92f5dc95ecdc476fd2f24` | 72、254、269、292、374、405、530 |
| `_app_part_10_handler_main.py` | `766c5cae763732a1351e07f4f764410545edf8420e06f1c6a8fde1883a1b637c` | 20–110、140–241、280–399、3340–3535 |
| `_app_part_05_worker_results.py` | `039ab111c3374f6e633bbb5e60a38cdbbcc5b2f519e354f280609c15adb55d90` | 13–47、368–416、462–696、733–736、1142–1181 |
| `_app_part_04_scan_audit_bundle.py` | `990bb432fdf12824dace9ac5e5da72b1320591df6f1738c08465dae1fd02d06e` | progress/public reviewRun/debug DTO构造 |
| `_app_part_01_bootstrap_state.py` | `abfd37b11b78a73c3f9cb7052af3da79042e5bf646148f8ea82be12fdea6e6d5` | job/scan statuses与lost映射 |
| `pullwise_server/db.py` | `1cc33a111c06eed52c26c5e414d031cf86e6d8269489cb57dc9b65b0077a5a19` | event/artifact/result idempotency和reaper |
| `tests/test_worker_pull_routes.py` | `f07c16e7a8287f5b1b8a16ec9094a1015f2892d6fa2b5dcbfbcd805670d9819c` | 282、351、372、923、1010、1390、2068、2168、2240、3618、5435、6613 |
| `tests/test_review_worker_protocol_v1.py` | `53e3fda1cc2af09fb770b4e3921ca2233eaa8dabc0371153079e56e3b7356099` | 14–162、760–1061 |
| `tests/test_cancellation_handshake.py` | `ef6a10348ecdfcbaafc6541ecbdc81fd7b7dd2be17992ac7ded7fd94e9dc51a0` | active cancel、409、late receipt fixtures |
| `pullwise-web/src/lib/pullwise-data.js` | `40d45c8bf55ffc05faf917d53a8fa47aa9ce57af05f40562dd06f621a0b4c144` | status/DTO/artifact URL normalization |
| `pullwise-web/src/screens/flow.jsx` | `207dedf59a9467f04bc3b17af46fe948b111d833747dbd0c49831d7c072e2541` | progress steps与debug action展示 |

当前 Server event type冻结为：`run_started/phase_started/progress_updated/phase_completed/phase_failed/phase_retrying/phase_degraded/codex_app_server_started/codex_app_server_failed/codex_thread_started/codex_turn_started/codex_turn_completed/codex_turn_failed/approval_declined/artifact_created/artifact_uploaded/qa_passed/qa_failed/run_cancel_requested/run_cancelled/run_completed/run_failed/run_partial_completed`。

当前 artifact kind冻结为：`report.human/report.agent/coverage/qa/token_budget/repo_inventory/repo_map/risk_routing/bundle_plan/cluster_result/validation_result/raw_reviewer_output/verified_reviewer_output/codex_event_log/worker_log/progress_log/error_report/intent_map/intent_test_plan/intent_test_source/intent_test_result/intent_test_output/disposable_test_patch/debug_bundle`。
...
6. 每次提交只包含当前Slice，附schema/fixture/test/rollback证据。
