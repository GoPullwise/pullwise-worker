...
Slice 2 只接入了读取 legacy marker/outbox 的内存投影。关闭 shadow bridge 即可回到
原有 legacy 行为；`agent-kernel/` 可保留为只读诊断数据，不要求数据库降级。
...
才可携带新 fence/epoch 接管。资格细节及最终 terminal authority/precedence 仍由 D9/D10
决定。当前 S2 reducer 的相反行为只是 pre-D8 shadow scaffold；在 S4 gate 清零前不得
