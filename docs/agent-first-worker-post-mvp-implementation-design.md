...
D8 已冻结 lease-loss 的 Task/Attempt 分层，机器 decision register 当前只能询问 `active_decision_id=D9`。本文中涉及“无合法 successor 后由谁终态化、终态 linearization point 和 outcome precedence”的既有候选描述，在 D9/D10 分别 resolved 前都不构成实现授权。
...
14. 外层 lease 丢失只终止旧 ownership：Task 保持 `ACTIVE|FINALIZING` 且 terminal/result 不变，精确旧 transport/native Attempt 与 actor/session 永久 fenced，并记录独立 transport abandonment 事实；只有满足显式恢复资格谓词的 successor 可接管。Lease loss 本身不是 Task terminalization authority。
...
下述 `commit_server_terminal_result` 只是待 D9/D10 闭合的候选接口轮廓，不授权实现 Task terminalization authority、linearization point 或 outcome precedence。D8 只允许 lease/worker loss 原子记录独立 transport abandonment、永久 fence 精确旧 transport/native Attempt 与 actor/session，并保持 Task `ACTIVE|FINALIZING` 及 terminal/result 不变。
...
Lease/worker loss 本身不得调用该方法或产生 `WORKER_LOST` Task terminal。空 Effect Ledger 时，`WORKER_LOST` 只可保留为“恢复资格已机械判定不存在或耗尽”之后的候选 Server-owned transport terminal，仍须等待 D9 确认 authority/linearization、D10 确认 precedence，且不生成 Worker TaskResult；有任何 effect row 时更不能用 pure lost 吞掉事实。资格仍待判定时必须保持 Task `ACTIVE|FINALIZING`、terminal/result 不变，并让公开 transport projection 表达 lost。
...
| `terminal` + `terminal.kind=worker_lost` | object或null | null | object/null | 仅在恢复资格已证明不存在/耗尽且 D9/D10 闭合的后续终态事务可产生；effects.total必须0，result/digest不存在，evidence unavailable reason=`WORKER_LOST` |
...
旧scan `status`保留业务兼容；transport lost不再伪装为Worker FAILED TaskResult，而在`agentTask.transport.status=lost`表达，同时 Task lifecycle 保持 loss 前的 `active|finalizing`、旧 Attempt 显示 fenced，直到合法 successor 接管或 D9/D10 授权的后续终态事务完成。Web正式接受`cancel_requested/cancelling`，不得把它们normalize成queued。
...
Rollout：Server schema/storage → Server parse shadow → Worker advertisement → grant仅内部tenant → Server strict ingest → Web feature detection → 分批grant。Rollback只能关闭新lease grant；已grant run继续同mode或被明确 fence，绝不降级。Fence 只终止旧 ownership，不直接 terminalize Task；无 successor 时的后续终态仍须 D9/D10 授权。
...
- lease/worker loss 只产生独立 abandonment 与旧 transport/native Attempt/actor fence；公开 Task 保持 active/finalizing、terminal/result 不变，且 stale actor 全入口拒绝。无恢复资格证明时不得投影 `terminal.kind=worker_lost`。
...
现有 terminal outcome 顺序候选为：delete fence（不产生新result） > UNKNOWN effect → `TERMINATED_WITH_UNKNOWN_EFFECTS` > cancel且COMMITTED effect → `CANCELLED_WITH_EFFECTS` > cancel无effect → `CANCELLED` > safe PARTIAL > FAILED/BLOCKED > success。D10 尚未 resolved，因此该候选不是 precedence 实现授权；D9 未闭合前，表中进入 `TERMINAL` 的行也不是 terminal authority/linearization 授权。任何实现都不得把 lease loss 插入该候选顺序并直接终态化 Task。
...
- Server reaper在Worker消失时只可原子记录 abandonment、fence 精确旧 ownership 并保持 Task active/finalizing；随后可仅凭 Server ledger 触发独立 effect reconciliation 资格判断。任何 DISPATCHED/UNKNOWN 都不能被 pure lost 隐藏，恢复资格未耗尽或 D9/D10 未闭合时也不得选择 WORKER_LOST Task terminal。
...
跨 lease resume 是 D8 所称 successor 的唯一正向能力：旧 lease 丢失时先独立提交 abandonment/fence 并保持 Task `ACTIVE|FINALIZING`；resume 请求随后只能消费该既有事实，不能在 successor 事务里第一次 fence 或再次 transition 旧 Attempt。`transport-abandonment-record/v1` 的字段、identity、幂等 contract 仍是 `SPEC_GAP`，本节不得就地发明。
...
Sweeper 只 fence/revoke 未确认的 successor 并释放未消费 slice；它不能因 successor 超时就终态化 Task。若仍要尝试另一个 successor，必须重新满足显式恢复资格与版本/fence；若资格耗尽，后续终态仍等待 D9/D10。

### 8.6 拒绝分类

- cancel先赢 → `RESUME_CANCELLED`，交给待 D9/D10 闭合的后续 Terminalization authority/precedence；resume 拒绝本身不写 Task terminal/result。
- budget/deadline不足 → `RESUME_BUDGET_INSUFFICIENT`，记录恢复资格拒绝；诚实 PARTIAL/FAILED 只能由 D9/D10 授权的后续终态事务选择。
...
- result被拒时fragment保持quarantine且不公开；只有 Task 后来经 D9/D10 授权的 Server Reconciler 终态事务完成后，Composer才可用权威snapshot生成`partial/RESULT_NOT_ACCEPTED`。Lease loss/abandonment 本身不满足该条件。
...
任何版本rollback都不得删除新schema数据或让active新mode run降级。必要时只先 fence 旧 ownership；Reconciler 的后续诚实 terminalization 必须等 D9/D10 闭合，不能由 rollback 或 lease loss 隐式授权。
...
| lease-loss Task/Attempt分层与 qualified successor | D8；正向 successor 在 V1.3 | abandonment/fence/version 原子矩阵、stale actor 拒绝、resume predicate/crash tests |
