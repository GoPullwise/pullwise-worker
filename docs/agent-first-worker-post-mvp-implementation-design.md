...
```json
{
  ...
  "event_id":"evt_<32hex>",
  "native_event_kind":"task.accepted|attempt.started|agent.turn|tool.dispatch|tool.receipt|observation.committed|checkpoint.committed|gate.decided|task.terminalized",
  "task_id":"...",
  "task_version":12,
  "deletion_version":0,
...
- Server先验证size/hash/schema/tenant/task/scope；`(task_id,artifact_id)`相同exact bytes幂等，不同metadata或bytes冲突。单对象、单Task总量、schema递归深度和TTL由grant冻结。
...
3. 依selected capabilities验证debug descriptor/terminal receipt；未selected必须是`unsupported`，selected但缺失走明确availability分支。
4. insert immutable raw envelope/TaskResult/TaskResultCore/result receipt binding并pin reachable evidence。
5. CAS Task terminal version、review run和scan projection。
6. 写terminal event/outcome labels和prefix。
7. durable保存并返回`task-result-receipt/v1`；response包含accepted/duplicate/receipt和terminal versions。
...
Server创建`resume-operation/v1`，状态`PREPARED→COMMITTED|REJECTED|COMMITTED_THEN_REVOKED`。成功同一事务：锁Task/lease → 验证谓词 → 核销旧reservation → 预留新slice → fence旧attempt → 创建successor transport attempt/epoch和新grant → 保存同job/run lease、replay fence和canonical response bytes → commit。
...
Worker拿到successor后再验证response binding，且必须先发送一次successor heartbeat/confirm；Server再次检查replay fence并ACK后，才可载入ACK checkpoint和启动Agent/tool。native/owner epoch继续递增。新lease不能延长absolute deadline或总budget。
...
V1.4完成门通过后，Worker才广告`worker_debug_fragment@1.0`；它依赖`agent_task_protocol@1`，Server同时必须实现snapshot/composer/public projection。只会上传fragment但没有Server Composer的部署不得offer/select该能力。
...
capture kind：startup/checkpoint/terminal/crash/postlude。普通capture必须有当前grant。所有upload成功先返回`fragment-upload-receipt/v1`，字段固定为`receipt_id/fragment_ref/task_id/deletion_version/run_id/transport attempt+epoch/native attempt+epoch/capture_kind/snapshot_seq/captured_at/accepted_at`。exact fragment identity/bytes重试返回同一receipt；同identity不同bytes为`DEBUG_RECEIPT_CONFLICT`。

startup/checkpoint/crash/postlude receipt的`task_result_core`必须是AvailabilityRef `not_applicable`。terminal fragment只能在TaskResultCore后生成，descriptor带完整ContentRef、identity和core digest；Server校验后签发独立`terminal-fragment-receipt/v1`，在普通receipt字段外强制`task_result_core_digest=available`并绑定grant/task/deletion/epochs。普通receipt不能冒充terminal receipt。
...
任一不足只能`partial`并给穷举reason code；最小registry为`IDENTITY_MISMATCH|TASK_VERSION_MISMATCH|EVENT_PREFIX_MISMATCH|SERVER_BOUND_EVENT_MISSING|CHECKPOINT_NOT_ACKED|FRAGMENT_MISSING|SNAPSHOT_MISSING|RESULT_RECEIPT_MISMATCH|COMPONENT_HASH_MISMATCH|REDACTION_FAILED`。多个reason按该固定顺序输出，不能凭时间接近或sequence大小猜complete。
...
Worker diagnostics→capture→assembly→public projection是机械映射：

| 条件 | availability | completeness | URL/hash/size | reasonCode |
|---|---|---|---|---|
| capability未selected | `unsupported` | null | 必须null | `CAPABILITY_NOT_SELECTED` |
| selected且run非终态、尚未compose | `pending` | null | 必须null | null |
| terminal且Worker明确无fragment | `absent` | null | 必须null | `FRAGMENT_NOT_CAPTURED` |
| upload失败/terminal receipt无效 | `upload_failed` | null | 必须null | 稳定upload/receipt code |
| Composer失败且没有可验证assembly | `composition_failed` | null | 必须null | 稳定composer code |
| 生成partial assembly | `available` | `partial` | 指向该immutable partial assembly | 9.4 reasons |
| 生成complete assembly | `available` | `complete` | 指向该immutable complete assembly | null |
| retention已过期 | `expired` | null | 必须null | `RETENTION_EXPIRED` |
| tombstone/delete | `deleted` | null | 必须null | `DELETED` |
...
legacy dynamic bundle只存在旧字段/路由并显式`kind=legacy_combined`；它不能写入`agentTask.debugBundle`、不能让new availability变available、不能使用new assembly SHA。迁移期间Web可并列显示`legacyDebugBundle`，但必须标“legacy/非完整”。任何状态都不fallback audit bundle，audit URL也不能作为legacy alias。
...
- 普通receipt/terminal receipt类型不可互换；terminal result/receipt无hash循环，相同capture/result重试bytes不变。
- debug availability投影表每格、partial URL、legacy alias和无audit fallback都有三端fixture。
