...
- `agent-event/v1`及event response/prefix fixtures
...
数组按ID排序、ID唯一、range合法。V1.0 binary不得提前列出`same_run_resume`或`worker_debug_fragment`；它们分别在V1.3/V1.4完成门通过后加入，且依赖同major的`agent_task_protocol`。Register只返回offer，不签发执行权：
...
```json
{
  ...
  "event_id":"evt_<32hex>",
  "native_event_kind":"task.transition|attempt.transition|interaction.transition|server.cancel|server.policy_changed|server.deleted|agent.turn|tool.dispatch|tool.receipt|observation.committed|checkpoint.committed|gate.decided|task.terminalized",
  "task_id":"...",
...
所有Worker控制transition要求`new_task_version=previous_task_version+1`，Server在同一事务CAS current Task version/lifecycle/desired state、更新Attempt/interaction、append event并更新prefix；exact transition ID重试返回原receipt。普通telemetry不改变Task/Attempt，只允许`observed_task_version=current`。Task/Attempt/interaction未列出的边按共享state machine拒绝。
...
四个digest是TaskResult的sibling，不属于被hash对象。Server必须从已上传bytes自行重算PreGate/final EvidenceClosure、TaskResultCore（只删`/diagnostics/worker_debug_fragment`）和完整TaskResult，校验task/scan、run/job/lease、epochs、outcome→legacy status映射、Requirement覆盖和Effect状态。V1.0尚未selected debug，`diagnostics.worker_debug_fragment`必须为`unsupported`；V1.4 selected后才验证terminal receipt。
...
2. 从quarantine解析/验证全部ContentRef、PreGate和final closure；尚不pin。
3. 依selected capabilities验证debug descriptor/terminal receipt；未selected必须是`unsupported`，selected时`uploaded`分支必须同时携带`server_fragment_ref`和`terminal_receipt_id`，`local_only|unavailable`分支必须令二者同时为null。
...
5. insert immutable raw envelope/TaskResult/TaskResultCore；若是`uploaded`分支，对`terminal-fragment-receipt/v1.bound_task_result_digest`执行唯一`null → task_result_digest` CAS；然后insert result receipt binding并pin reachable evidence。
...
Terminal receipt的相同`receipt_id/core_digest/full_digest/canonical envelope`重试沿用同一binding和result receipt；receipt已绑定不同full digest时409 `TERMINAL_RECEIPT_ALREADY_BOUND`。若结果事务在CAS前后任一步失败，整笔回滚，receipt仍为未绑定且fragment仍在quarantine，不产生可下载terminal assembly。
...
新增`POST /v1/agent-tasks/{task_id}/effects/{effect_id}/transitions`和read-only exact lookup。`effect-transition/v1`必须带grant/task/deletion/transport/native fence、`transition_seq`、expected Server effect version、from/to、idempotency key、request digest和相应receipt refs。Server在任何写前验证selected`effect_ledger_r3`、状态边和所有fence。

Server规则：

- `(effect_id,transition_seq)` exact canonical bytes幂等；同seq不同bytes、跳seq、from/version不匹配为409且零partial write。
...
幂等键为`effect_id + expected_effect_version + query_operation_id + receipt_digest + target_state`；同key exact重试返回原transition receipt，不同bytes冲突。单一事务检查deletion/effect version、写receipt/transition、更新Task RECONCILING projection和event prefix。Delete/tombstone先赢则拒绝Task写并只留不含tenant payload的security audit；transition先赢后delete仍可随后删除bytes，历史digest不改。
...
Server还返回authoritative `last_accepted_event_sequence/next_event_sequence/event_prefix_digest`，使用5.3.1的event identity/prefix算法解决MVP event ambiguity。Worker恢复时采用Server watermark，不回退sequence；只允许exact event ID/sequence/canonical bytes重放，不同digest冲突。

Checkpoint ACK body本身作为`server-resume-checkpoint-watermark/v1` durable保存；heartbeat response丢失时Worker用同generation/hash重发，Server返回逐字节相同ACK。若其间cancel/revoke/delete改变fence，Server返回相应拒绝且不把旧ACK升级为恢复授权。
...
Server创建`resume-operation/v1`，状态`PREPARED→COMMITTED|REJECTED|COMMITTED_THEN_REVOKED`。成功同一事务：锁Task/lease → 验证谓词 → 核销旧reservation → 预留新slice → fence旧attempt → 创建successor transport attempt/epoch和新grant → 保存同job/run lease、replay fence和canonical response bytes → commit。successor lease状态固定为`PENDING_CONFIRM`，grant authorization state固定为`pending_confirm`，response含`confirm_deadline_at=min(now+60s,resume_until,grant.expires_at,absolute_deadline-terminal_reserve)`。

`replay_fence`至少绑定`task_version/deletion_version/desired_state/security_revocation_epoch/successor_grant_id/successor_lease_epoch/absolute_deadline`。相同operation ID的后续请求先锁行并重验fence：
...
Confirm单一事务重验replay fence、Task/deletion/security versions、deadline/budget、successor identity和ACK checkpoint；然后绑定native/owner identity、令lease/grant authorization state变`ACTIVE/active`、Task version+1，并按5.3.1 append `attempt.transition` event/prefix，durable保存`resume-successor-confirm-receipt/v1`。只有ACK receipt返回后Worker才把本地pending Attempt变PREPARING并恢复runtime。
...
V1.4完成门通过后，Worker才广告`worker_debug_fragment@2.0`；它依赖`agent_task_protocol@1`，Server同时必须实现snapshot/composer/public projection。只会上传fragment但没有Server Composer的部署不得offer/select该能力。MVP的`worker-debug-fragment/v1`仅作只读迁移输入，不能由V1.4新capture产生，也不能单独满足complete条件。
...
startup/checkpoint/crash/postlude receipt的`task_result_core`必须是AvailabilityRef `not_applicable`。terminal fragment只能在TaskResultCore后生成，descriptor带完整ContentRef、identity和core digest；Server校验后签发独立`terminal-fragment-receipt/v1`，在普通receipt字段外强制`task_result_core_digest=available`并绑定grant/task/deletion/epochs，且初始化`bound_task_result_digest=null`。该字段只能由5.4 Result accept事务做一次性CAS。普通receipt不能冒充terminal receipt。
...
| 优先级 | 条件 | availability | completeness | URL/hash/size | reasonCode |
|---:|---|---|---|---|---|
...
| 4 | 已验证complete assembly | `available` | `complete` | 指向该immutable assembly | null |
| 5 | 已验证partial assembly | `available` | `partial` | 指向该immutable assembly | 9.4 ordered reasons |
| 6 | Composer已尝试且失败、没有可验证assembly | `composition_failed` | null | 必须null | 稳定composer code |
| 7 | terminal upload失败或terminal receipt无效、且没有partial assembly | `upload_failed` | null | 必须null | 稳定upload/receipt code |
...
| 10 | selected且run非终态、尚未compose | `pending` | null | 必须null | null |
...
legacy dynamic bundle仍只存在旧字段/路由并显式`kind=legacy_combined`；它不能写入`agentTask.debugBundle`、不能让new availability变available，也不能冒充`source=agent_assembly`。迁移期间Web可并列显示`legacyDebugBundle`，但必须标“legacy/非完整”。任何状态都不fallback audit bundle，audit URL也不能作为legacy alias。
...
- debug ordered projection每格、`local_only`、普通fragment无terminal receipt、result被拒、server-only snapshot、orphan capsule、partial URL都有三端fixture。
