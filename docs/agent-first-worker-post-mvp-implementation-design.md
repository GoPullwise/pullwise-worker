...
所有远期版本宣称为已经闭合的 implementation design；机器 decision register 的唯一活动问题
现为 `active_decision_id=D22`。D26 只确定文档成熟度与开工边界，不授权 runtime、schema、
...
core/transport 双 digest 的无环关系；D26 已冻结上述 maturity 分流。D22 是当前唯一活动问题。
完整矩阵在受控规范中冻结前，本文既有候选顺序仍不构成实现授权。
...
- Worker、Server、Web 对同一 exact-pin current package 的发布组合、grant on/off/revoked/partial 和 legacy-absence 组合全部自动化；不得用 old/new compatibility matrix 代替 clean-break 证据。
...
5. 运行 unit/contract/property/crash/security、exact-pin current-package 发布组合与 legacy-absence matrix。
6. 记录ControlPlaneDigest、migration/rollback证据、指标门，以及MVP第3.2节要求的新增/修改文件行数、baseline变化和模块化例外。
7. 只在第3节依赖表列出的全部硬依赖完成门通过后启用本版能力；未列为硬依赖的编号中间版可以跳过。具体地，V1.2可在V1.0稳定后独立于roadmap V1.1交付，但不得使用任何未由独立设计和决议授权的动态角色。
...
crash fixtures、rollout、rollback、DoD、owner 与数值门。设计必须接受独立决议并明确协调切换
边界，之后才能把获批范围转为 implementation baseline 并按第17.1节执行。
