...
- Normalize reporter `appendix.weak_findings` and validator `weak_findings` into top-level `report.agent.json.appendix_findings` before building the stable envelope. `summary.finding_counts.weak_appendix` must preserve that canonical appendix count even though weak findings are not issue-eligible main findings.
